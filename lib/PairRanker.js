var PairRanker = function (options) {};

//Stores each Unique pair in an object and keeps track of the average rating
PairRanker.prototype.getPairRanking = (candidates,pairRanks, ctrl, logger)=>{
  ...
  pairRanks = PairRanker.cleanPairingArray(pairRanks, logger);	
  ...
  while (!check && k < 5 && candidates[0].rate > parseFloat(process.env.minimalProfit)){
	  k++;
	  check =	PairRanker.getTopPairs(candidates[k],pairRanks, logger);
...
////
PairRanker.cleanPairingArray = (pairRanks, logger) =>{
...
////
PairRanker.getTopPairs = (pairToCheck, pairRanks, logger) =>{
  ...
  var rate = 0;
  for (i=0;i<pairsToCheck.length;i++){
